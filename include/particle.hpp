...
class Particle {
    ...
    private:
        ...
        /* Stored private informations for the Particle class are the mass, position, velocity and acceleration */
...
/* Random generator used for initialisation purposes. If one wants to use a manual seed, seedIsRandom must be set to "false". If
it is set to "true" the seed will be random. This function is stored here because Particle items call it. */

double randomValueGenerator(double minRandomValue, double maxRandomValue, bool seedIsRandom = false);
